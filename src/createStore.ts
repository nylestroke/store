 ...
 */
export function createStore<S extends Record<string, any>>(initialState: S): Store<S> {
    ...
    return {
        get<K extends keyof S>(key: K): S[K] {
            ...
            if (!exists) {
                throw new Error(`Value for provided key (${JSON.stringify(key)}) does not present`);
            }
        ...
        clear(): void {
            ...
            Object.keys(store).map((key: string): void => { // Work-around from type-safety issues
        ...
        empty<K extends keyof S>(key: K): void {
            ...
            Object.keys(store).map((index: string): void => { // Work-around from type-safety issues
                if (index === key) {
                    const nullObj = { [key]: null };
                    Object.assign(store, nullObj);
                }
